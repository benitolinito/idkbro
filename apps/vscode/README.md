...
Open a clean clone of the same Git repository at the host's base commit before hosting or joining. Hosts need Git and the selected agent CLI. Claude is an external-binary MVP: enable `multicode.experimentalClaude`, install the Claude CLI, and configure an Anthropic API key. Claude steering stays hidden until its active-turn behavior is verified; queue and interrupt remain available. Bootstrap uses a verified checkpoint; live UTF-8 text and file operations use the host's durable Yjs/manifest authority. Room changes appear directly in both users' local checkouts and remain there after leaving.
